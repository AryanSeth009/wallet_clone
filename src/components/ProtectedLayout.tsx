...
export default function ProtectedLayout({
  ...
  useEffect(() => {
    ...
    if (!loading && !user && pathname && protectedRoutes.includes(pathname)) {
      router.push('/home');
    }
    // Redirect to home if accessing auth routes while logged in
    if (!loading && user && pathname && hideNavigationRoutes.includes(pathname)) {
      router.push('/');
    }
  ...
  if (pathname && hideNavigationRoutes.includes(pathname)) {
    return <>{children}</>;
  }
