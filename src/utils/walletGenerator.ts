...
export class WalletGenerator {
  ...
  async getTransactionHistory(address: string, limit: number = 10): Promise<TransactionDetails[]> {
    ...
    // Fallback when Etherscan API is not available
    try {
      // In ethers v6, getHistory is no longer available
      // Return empty array as we can't get history without Etherscan
      console.warn('Transaction history not available without Etherscan API');
      return [];
    } catch (error) {
