...
export default class WalletService {
  ...
  async sendTransaction(
    ...
    try {
      ...
      // Send transaction
      const transaction = await wallet.sendTransaction(txWithGas);
  ...
  async importWalletFromPrivateKey(userId: string, privateKey: string, name?: string): Promise<WalletAccount> {
    try {
      ...
      const walletName = name || `Imported Wallet ${new Date().toLocaleDateString()}`;
  ...
  async importWalletFromMnemonic(userId: string, mnemonic: string, name?: string): Promise<WalletAccount> {
    try {
      if (!userId || !mnemonic) {
        throw new Error('User ID and mnemonic are required');
      }
