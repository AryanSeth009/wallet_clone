...
interface WalletContextType {
  ...
  importWalletFromPrivateKey: (privateKey: string, walletName: string) => Promise<WalletAccount>;
  isLoading: boolean;
  error: string | null;
}
...
export const WalletProvider: React.FC<WalletProviderProps> = ({ children }) => {
  ...
  const importWalletFromPrivateKey = async (privateKey: string, walletName: string) => {
    try {
      if (!session?.user?.id) {
      ...
      setIsLoading(true);
      const newWallet = await walletService.importWalletFromPrivateKey(privateKey, walletName, session.user.id);
      if (!newWallet) {
  ...
  return (
    <WalletContext.Provider
      value={{ wallets, selectedWallet, setSelectedWallet, createWallet, importWalletFromPrivateKey, isLoading, error }}
    >
      {children}
    </WalletContext.Provider>
  );
